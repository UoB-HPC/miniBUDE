...
int main(int argc, char *argv[])
{
  ...
  float *resultsOpenACC = malloc(params.nposes*sizeof(float));
  float *resultsRef = malloc(params.nposes*sizeof(float));

  runOpenACC(resultsOpenACC);
  ...
  {
    if (fabs(resultsRef[i]) < 1.f && fabs(resultsOpenACC[i]) < 1.f) continue;

    float diff = fabs(resultsRef[i] - resultsOpenACC[i]) / resultsOpenACC[i];
    if (diff > maxdiff)
      maxdiff = diff;

    if (i < 8)
      printf("%7.2f    vs   %7.2f  (%5.2f%%)\n", resultsRef[i], resultsOpenACC[i], 100*diff);
  }
...
#pragma acc parallel loop gang vector\
            present(protein_s[:natpro_s], ligand_s[:natlig_s], \
                    forcefield_s[:ntypes_s], poses_0[:nposes_s], \
                    poses_1[:nposes_s], poses_2[:nposes_s], \
                    poses_3[:nposes_s], poses_4[:nposes_s], \
                    poses_5[:nposes_s], results[:nposes_s])
