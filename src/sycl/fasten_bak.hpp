...
template <size_t PPWI> class IMPL_CLS final : public Bude<PPWI> {
  ...
  struct AccessorIterator {
    ...
    explicit AccessorIterator(Accessor &accessor, int offset) : accessor(accessor), i_(offset) {}

    reference operator *() const { return accessor[i_]; }
    const AccessorIterator &operator ++() { ++i_; return *this; }
    AccessorIterator operator ++(int) { AccessorIterator copy(*this); ++i_; return copy; }
    const AccessorIterator &operator +=(int by) { i_+=by; return *this; }

    const AccessorIterator &operator --() { --i_; return *this; }
    AccessorIterator operator --(int) { AccessorIterator copy(*this); --i_; return copy; }


    value_type operator[](const difference_type &i) const { return accessor[i_ + i]; }

    difference_type operator-(const AccessorIterator &it) const { return i_ - it.i_; }
    AccessorIterator operator+(const value_type v) const { return AccessorIterator(i_ + v); }

    bool operator ==(const AccessorIterator &other) const { return i_ == other.i_; }
    bool operator !=(const AccessorIterator &other) const { return i_ != other.i_; }
    bool operator < (const AccessorIterator &other) const { return i_ < other.i_; }
  ...
  static void fasten_main(sycl::handler &h,                                                           //
    ...
    h.parallel_for<bude_kernel_ndrange<PPWI>>(sycl::nd_range<1>(global, wgsize), [=](sycl::nd_item<1> item) {
      ...
      for (size_t il = 0; il < ligands.template get_count(); il++) {
        ...
        std::for_each(begin(proteins), end(proteins), [](Atom &p) {

        });

        // Loop over protein atoms
        for (size_t ip = 0; ip < proteins.template get_count(); ip++) {
